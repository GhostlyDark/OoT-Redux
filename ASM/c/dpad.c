...
extern uint8_t CFG_SET_SHIELD_ENABLED;
...
void handle_dpad() {
	...
	if (!z64_file.link_age) {
		...
		if (pad_pressed.du && z64_file.hylian_shield && z64_file.mirror_shield && CFG_SET_SHIELD_ENABLED) {
	...
	if (z64_file.link_age) {
		//if (pad_pressed.dl && z64_file.items[Z64_SLOT_BEANS] == Z64_ITEM_BEANS && z64_file.ammo[Z64_ITEM_BEANS] > 0)
		...
		if (pad_pressed.du && z64_file.deku_shield && z64_file.hylian_shield && CFG_SET_SHIELD_ENABLED) {
			if (z64_file.equip_shield == 2)
...
void draw_dpad() {
	...
	if (z64_file.link_age) {
		/*if (z64_file.items[Z64_SLOT_BEANS] == Z64_ITEM_BEANS && z64_file.ammo[Z64_ITEM_BEANS] > 0 && z64_file.link_age) {
